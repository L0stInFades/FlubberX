...
const ORIGINAL_MAX_SEGMENT_LENGTH = 10;
const DEFAULT_ENDPOINT_EPSILON = 1e-4;

/** EDYT helpers pin these explicitly so `interpolate` can keep original defaults. */
const EDYT_MAX_SEGMENT_LENGTH = 0.005;
const EDYT_PRECISION = 6;
...
export function interpolate(
  ...
  toShape: ShapeInput,
  {
    maxSegmentLength = ORIGINAL_MAX_SEGMENT_LENGTH,
    string = true,
  ...
  const out = canSnap
    ? (t: number) => {
        ...
        if (tt < endpointEpsilon && typeof fromShape === "string") return fromShape;
        if (1 - tt < endpointEpsilon && typeof toShape === "string") return toShape;
        return core(tt);
      }
    : core;
...
export function createShapeMorphInterpolator(
  ...
  return interpolate(fromPath, toPath, {
    maxSegmentLength: EDYT_MAX_SEGMENT_LENGTH,
    clamp: true,
    precision: EDYT_PRECISION,
    ...options,
    string: true,
  });
}
...
export function interpolateRing(
  ...
  const { string = true, clamp = false, precision = null } = options;
