...
describe("flubber-edyt interpolate", () => {
  ...
  it("keeps EDYT-tuned subdivision denser than original interpolate defaults", () => {
    const a = "M0,0L1,0L1,1L0,1Z";
    const b = "M0,0L2,0L2,2L0,2Z";
    const edytMid = createShapeMorphInterpolator(a, b)(0.5);
    const originalMid = interpolate(a, b)(0.5);
    expect(edytMid.split("L").length).toBeGreaterThan(originalMid.split("L").length);
  });
