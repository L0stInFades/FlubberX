import { INVALID_INPUT_ALL } from "../src/errors.ts";
import { interpolateAll } from "../src/multiple.ts";
import type { Ring, ShapeInput } from "../src/types.ts";
...
tape("all with array inputs", (test) => {
  const interpolate = interpolateAll(
    [shapes.square1(), shapes.square2()],
    [shapes.triangle1(), shapes.triangle2()],
    { maxSegmentLength: Infinity },
  );
  const interpolateArray = interpolateAll(
    [shapes.square1(), shapes.square2()],
    [shapes.triangle1(), shapes.triangle2()],
  ...
  test.assert(interpolate.every((fn) => isPathStr.test(String(fn(0)))));
  test.assert(interpolate.every((fn) => isPathStr.test(String(fn(0.5)))));
  test.assert(interpolate.every((fn) => isPathStr.test(String(fn(1)))));
...
tape("all with string inputs", (test) => {
  const interpolate = interpolateAll([square, halfcircle], [bezier, square], {
    maxSegmentLength: 10,
  });
  const interpolateArray = interpolateAll(
    [square, halfcircle],
...
tape("all with array inputs single", (test) => {
  const interpolate = interpolateAll(
    [shapes.square1(), shapes.square2()],
    [shapes.triangle1(), shapes.triangle2()],
  ...
  test.assert(isPathStr.test(interpolate(0)));
  test.assert(isPathStr.test(interpolate(0.5)));
  test.assert(isPathStr.test(interpolate(1)));
  ...
  interpolateArray(0.5).forEach((ring) => {
...
tape("all with string inputs single", (test) => {
  const interpolate = interpolateAll([square, halfcircle], [bezier, square], {
    single: true,
  });
  const interpolateArray = interpolateAll(
...
tape("errors", (test) => {
  const err = new RegExp(INVALID_INPUT_ALL.slice(25, 50));
  ...
  test.throws(
    () => interpolateAll(1 as unknown as ShapeInput[], [] as ShapeInput[]),
    err,
  );
