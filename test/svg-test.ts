...
import { pathStringToRing, splitPathString, toPathString } from "../src/svg.ts";
...
tape("pathStringToRing general", (test) => {
  test.inDelta(
...
tape("interpolate preserve string at t=0, t=1", (test) => {
  const interpolator = interpolate(halfcircle, bezier);

  test.deepEqual(interpolator(0), halfcircle);
  test.deepEqual(interpolator(1), bezier);

  const ringInterpolator = interpolate(halfcircle, bezier, { string: false });
  const first = ringInterpolator(0);
  const last = ringInterpolator(1);
...
tape("pathStringToRing preserve", (test) => {
...
tape("pathStringToRing produces at least 3 points", (test) => {
