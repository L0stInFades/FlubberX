...
tape("separate/combine single preserve string at t=0, t=1", (test) => {
  const separator = separate(square, [halfcircle, bezier], { single: true });
  const combiner = combine([halfcircle, bezier], square, { single: true });
...
tape("separate/combine preserve string at t=1", (test) => {
  const separator = separate(square, [halfcircle, bezier]);
  const combiner = combine([halfcircle, bezier], square);
